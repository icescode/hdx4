...
build:
	...
	go build -o hdx-flac2wav cmd/hdx-flac2wav/hdx-flac2wav.go

	@echo "--- building HDX-STRUCT---"
	go build -o hdx-struct cmd/hdx-struct/hdx-struct.go

	@echo "--- building HDX-VOLMAKER---"
	go build -o hdx-volmaker cmd/hdx-volmaker/*.go

	@echo "--- building HDX-META---"
	go build -o hdx-meta cmd/hdx-meta/hdx-meta.go

	@echo "--- building HDX-PLAYER-DEBUG ---"
	go build -o hdx-player-debug cmd/hdx-player-debug/hdx-player.go

	@echo "--- building HDX-SERVER ---"
	go build -o hdx-server cmd/hdx-server/*.go

	@echo "--- building HDX-CLIENT ---"
	go build -o hdx-client cmd/hdx-client/hdx-client.go
	@echo "--- BUILD END ---"
